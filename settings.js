...
function initSettings() {
    chrome.storage.sync
        .get('siteUrl')
        .then(data => (document.getElementById('siteUrl').value = data.siteUrl))

    chrome.storage.sync
        .get('theme')
        .then(data => (document.getElementById(data.theme).checked = true))
...
function updateCheck(key) {
    chrome.storage.sync
        .get(key)
        .then(data => (document.getElementById(key).checked = data[key]))
}
...
function fillSemesterSelect() {
    document.getElementById('dataRequestButton').innerHTML =
    ...
    if (siteUrl !== undefined) {
        fetch(`https://${siteUrl}/services/data.php?q=dataPremièreConnexion`)
            ...
            .then(data => {
                if (data.redirect) {
                    alert(
                        'Merci de vous connecter pour pouvoir récupérer les données.'
                    )
                    document.getElementById('dataRequestButton').innerHTML =
                        'Merci de vous connecter pour pouvoir récupérer les données.'
                    return
                ...
                document.getElementById('dataRequestButton').innerHTML =
                    'Données récupérées'
                document.getElementById('dataRequestButton').className =
                ...
                for (const semester of data.semestres) {
                    ...
                    document
                        .getElementById('semesterSelection')
                        .appendChild(button)
                }
            })
    }
...
function semesterSelected(event) {
    document.getElementById('semesterSelectionButton').innerText =
        event.target.innerText

    document.getElementById('semestersLoading').style.display = 'block'
    if (siteUrl !== undefined) {
        fetch(
            `https://${siteUrl}/services/data.php?q=relevéEtudiant&semestre=${event.target.value}`
        )
            .then(response => response.json())
            .then(data => {
                if (data.redirect) {
                ...
                document.getElementById('semestersLoading').style.display =
                    'none'
...
function buildRessourcesWeightSelector(ues, currentSemester) {
    ...
    for (const key of Object.keys(ues).sort()) {
        ...
        for (const key of Object.keys(ueData.ressources).sort()) {
            ...
            ressourceDiv.className =
                'list-group-item d-flex justify-content-between align-items-center pt-0 pb-0 pe-0'
            ...
            ressourceWeightInput.value =
                ues[ueData.name].ressources[ressourceData.name].weight
            ressourceWeightInput.setAttribute('aria-label', 'Coef')
            ressourceWeightInput.setAttribute(
                'aria-describedby',
                `${ueData.name}-${ressourceData.name}`
            )
            ressourceWeightInput.onchange = event => {
                ressourceWeightChange(
                    event,
                    ues,
                    ueData.name,
                    ressourceData.name,
